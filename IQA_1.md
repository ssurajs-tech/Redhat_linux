...
02. How to remove files older than 7 days by creating the cron jobs to run every night ?
	...
	# crontab -e  --> '0 2 * * * /bin/bash find /var/log/messages -type f -mtime +7 -exce rm -rf {} \;'
	# crontab -l 
...
19. Find and remove file over a 100 MB ?
	...
	## ✅ 1. Just Find Files Over 100 MB
	
		```
		find /var/log -type f -size +100M -exec ls -lh {} \;
		```
		
		> Note: Use `100M` (not `100MB`, because `MB` is not valid for `find`).
		
	---
	
	## ❗ 2. Remove Files Over 100 MB (with confirmation)
	
		```
		find /var/log -type f -size +100M -exec rm -i {} \;
		```
		
		This will ask you yes/no for each file.
		
	---
	
	## ⚠️ 3. Remove Files Over 100 MB (force delete — BE CAREFUL)
	
		```
		find /var/log -type f -size +100M -delete
		```
		
		This will delete instantly without asking.
		
	---
	
	## ⭐ Best Practice for Logs
	
		Instead of deleting directly, compress or rotate using `logrotate`.
...
20. Get list of users who logged in today ?
	...
	# ✅ Correct & Reliable Commands
	
	## 1. Get today’s date in the format used by `last`
	
		`last` shows dates like this:
		...
		So we extract today’s date in the same format:
		
		```
		today=$(date +"%b %e")
		```
		
		(Example output: `Nov 24`)
		
	---
	
	## 2. List users who logged in today
	
		```
		last -F | grep "$today"
		```
		
		This will show login entries that match today’s date.
	...
	* `last` reads from `/var/log/wtmp`, so it only shows successful logins, not failed ones.
	* This includes `tty`, `pts`, and SSH logins.
	
	---
	
	# 📌 Optional: Unique list of usernames who logged in today
	
	```
	last -F | grep "$(date +"%b %e")" | awk '{print $1}' | sort -u
	```
	
	---

21. ✅ How do you investigate when a website does not load?

	When a website is not loading, I troubleshoot in a layered approach: service → port → network → DNS → application → logs → backend.
	...
	# 🔹 2. Verify service is listening on the correct port
		...
		If the service is not listening, fix configuration or restart.
	...
	# 🔹 3. Check firewall rules
		...
		And security groups (AWS/Cloud):
		
		* Allow port 80/443
		* Allow backend ports if needed
		
	---
	
	# 🔹 4. Check SELinux (common on Red Hat)
		...
		If it works → fix with proper SELinux contexts:
	...
	# 🔹 5. Check the web server logs
		...
		* Permission issues
		* File not found
		* Backend failure (502/504)
		
	---
	
	# 🔹 6. Check document root & files
	
		Verify:
		
		* `index.html` or `index.php` exists
		* Correct path is configured in config file
	...
	# 🔹 7. Test from server itself
		...
		If curl works locally but not externally → network/firewall issue.
	...
	# 🔹 9. Check reverse proxy or load balancer (if used)
	...
	# 🔹 10. Validate backend application
	
		For dynamic sites (Node.js, Python, Java, PHP):
	...
	# 🎯 Short and crisp interview-ready answer
	
	You can say this in under 30 seconds:
	
	> “I follow a layered approach. First I check if the web server (nginx/httpd) is running and listening on the correct port using systemctl and ss.
	> Then I check firewall, SELinux, and log files for errors.
	> Next I verify that the index file exists in the document root and the configuration points to the right directory.
	> If backend services are involved, I check whether they are running and reachable.
	> Finally, I validate DNS, load balancer health, SSL certificates, and test using curl from the server.”
...
25. Troubleshooting User Login Issues: A Scientific Method Approach

	This document demonstrates how to apply the scientific method to troubleshoot a Linux user login problem. Here's a comprehensive step-by-step breakdown:
	
	---
	
	## Problem Overview
	
	After weekend maintenance, the `student` user cannot log into `servera`. The user reports:
	- Username: `student`
	- Password: `student`
	- Normally uses passwordless SSH from `workstation`
	...
	## Step 1: Reproduce the Problem (Gather Information)
	
		### Test SSH Login from Workstation
		```bash
		...
		Key Insight: Authentication works, but the session terminates instantly.
		
		### Test Console Login
		Attempt direct console login with password `student`.
		
		Observation: Same behavior—login succeeds but session closes immediately.
	...
	## Step 2: Verify System Functionality
		...
		Result: ✅ Root login works perfectly.
		
		Conclusion: The server itself is functional; the issue is specific to the `student` account.
	
	---
	
	## Step 3: Investigate the Student Account
	
		### Attempt to Switch to Student User
		```bash
		...
		Observation: The `su -` command fails to switch users. The prompt remains as `root`.
		
		Key Insight: Even root cannot establish a shell session as `student`.
	...
	## Step 4: Check Login History
		...
		Finding: The user previously logged in successfully, confirming the account worked before the weekend maintenance.
	
	---
	
	## Step 5: Examine Account Configuration
		...
		Critical Discovery: The shell is set to `/bin/false`!
		
		### Understanding `/bin/false`
		- `/bin/false` is a non-interactive shell
		- It immediately exits with a failure status
		- Used for service accounts that shouldn't have interactive access
		- This explains why connections close immediately
		
	---
	
	## Step 6: Create a Problem Statement
	
		> "Since the previous weekend, the student user can access servera but is unable to create a working shell session. 
		Because other users can start sessions on servera, and the student user can start sessions on other servers, the problem appears only with the student 
		use of servera."
		
		This statement:
		- Defines the scope clearly
		- Eliminates broader system issues
		- Focuses on the specific user-server combination
		
	---
	
	## Step 7: Form a Hypothesis
	
		Hypothesis: The `/bin/false` shell prevents interactive sessions. Changing it to `/bin/bash` (the user's preferred shell) will resolve the issue.
		
		Test Plan: Modify the shell configuration and attempt login again.
	...
	## Step 8: Test the Hypothesis
		...
		Result: ✅ Success! The user now has a working shell session.
	...
	## Step 9: Document the Solution
		...
		Test Record:
		- Action: Modified the user account to use `/bin/bash` in servera's local user configuration
		- Method: Used `chsh -s /bin/bash student`
		- Result: Success—student can now log in and create shell sessions
		
		Solution Statement (Inverse of Problem):
	...
	## Key Takeaways: The Scientific Method in IT
	
		1. Reproduce → Confirm the issue exists
		2. Gather Data → Collect symptoms and system information
		3. Isolate → Determine scope (specific to user/system/service)
		4. Hypothesize → Form educated guesses based on evidence
		5. Test → Verify each hypothesis systematically
		6. Document → Record findings and solutions
		7. Verify → Confirm the inverse problem statement is true
	...
	## Common Commands Used
		...
		| `lastlog -u user` 		| Check last login time 			|
		| `getent passwd user` 		| View user account details 		|
		| `chsh -s /shell user` 	| Change user's default shell 		|
	
26. What are the fields of  /etc/passwd file?


27. What is difference between .bash and .bashrc?
28. What details do you get when you run the finger command?
29. Name the three files which are automatically created inside any user’s directory when a user is added?
30. What is the command to view all currently logged in users?